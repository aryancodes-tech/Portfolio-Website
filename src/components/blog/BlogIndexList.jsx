...
const BlogIndexList = ({ items }) => {
  ...
  const standaloneItems = items.filter((item) => item.type === 'standalone')
        ...
        return (
          <div key={`series:${topic.entrySlug}`}>
            <button
              ...
              onClick={() => toggleTopic(topic.entrySlug)}
            ...
            <div id={`topic-${topic.entrySlug}`} className={isOpen ? 'block' : 'hidden'}>
              <ul className="divide-y divide-[hsl(var(--border))]">
                {topic.posts.map((post) => (
                  <li key={`${topic.entrySlug}:${post.postSlug}`}>
                    <Link
                      to={blogPostHref(topic.entrySlug, post.postSlug)}
